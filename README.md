# Henry · Data Analytics Portfolio

##  Financial Product Analysis Report

This repository contains a comprehensive financial product analysis report with data visualizations.

###  Available Reports
- **[Financial Product Analysis](./projects/financial_product_analysis.md)** - Comprehensive analysis of stocks, bonds, and funds
- **[RFM Customer Segmentation](./projects/rfm_analysis.md)** - Customer segmentation analysis
- **[Product Performance Analysis](./projects/product_analysis.html)** - E-commerce product analysis

###  Quick Access
- **Main Portfolio**: [main.html](./main.html)
- **GitHub Pages**: https://xiaomiao027-sys.github.io (once deployed)

###  Contact
- **Email**: xiaomiao027@outlook.com
- **GitHub**: [xiaomiao027-sys](https://github.com/xiaomiao027-sys)
- **LinkedIn**: [Hon Yue Cheung](https://www.linkedin.com/in/hon-yue-cheung-3a3a04386/)

###  Tech Stack
- **Python** - Data analysis and visualization
- **MkDocs** - Documentation generation
- **GitHub Actions** - Automated deployment

---

>  **Note**: GitHub Pages deployment is in progress. If the site is not accessible, please check the Actions tab for deployment status.
